...
def preprocess_file(file_path, mod):
    ...
    # Hit leniency x — matches sunny-rework C# getHitWindow300 + SunnySkill
    od = p[5]
    clock_rate = 1.5 if mod == "DT" else (0.75 if mod == "HT" else 1.0)
    anti_od = min(10.0, max(0, 10.0 - od))
    hw_val = 34 + 3 * anti_od
    hw_val *= clock_rate
    hw_val += 1e-6
    great_hit_window = (int(hw_val) + 0.5) / clock_rate
...
def calculate(file_path, mod):
    ...
    # C_arrV2: note count within 500ms including LN tails (matches sunny-rework C#)
    note_hit_times_v2 = sorted([n[1] for n in note_seq] + [n[2] for n in note_seq if n[2] >= 0])
    C_stepV2 = np.zeros(len(base_corners))
    for i, s in enumerate(base_corners):
        cnt = bisect.bisect_left(note_hit_times_v2, s + 500) - bisect.bisect_left(note_hit_times_v2, s - 500)
        C_stepV2[i] = cnt
    C_arrV2 = step_interp(all_corners, base_corners, C_stepV2)
    ...
    # Use C_arrV2 (heads + tails) for non-CL, matching sunny-rework C# code.
    effective_weights = C_arrV2 * gaps
    ...
    spikiness = np.sqrt(weighted_variance) / weighted_mean

    # --- Switches ---
    # C# code passes D_all (not effective_weights) as the weights parameter
    switches = other_params.switch(note_seq, tail_seq, all_corners, Ks_arr, D_all)

    # --- Variety ---
    variety_val = other_params.variety(note_seq, note_seq_by_column)
